		...
		do {
			...
			case 1:
				if (count < arrOfEmployee.length) {
					...
					arrOfEmployee[count] = new Manager(sc.next(), sc.next(), sc.nextInt(), sc.nextDouble(),
							sc.nextInt());
					System.out.println("\n Manager hired");
					count++;
				} else {
			...
			case 2:
				if (count < arrOfEmployee.length) {
					...
					arrOfEmployee[count] = new Worker(sc.next(), sc.next(), sc.nextInt(), sc.nextDouble(), sc.nextInt(),
							sc.nextDouble());
					System.out.println("\n Worker hired");
					count++;
				} else {
			...
			case 6: {
				...
				empNo = sc.nextInt() - 101;
				for (Employee temp : arrOfEmployee) {
					
					if (empNo < 0 && empNo > arrOfEmployee.length)
						System.out.println("\n Enter valid empNo");
					else {
						
						if (temp.getEmpId()-101 == empNo) {
							
							if (temp instanceof Manager) {
								System.out.println("\n Manager\'s Performance bonus: "+((Manager) temp).getPerformanceBonus());
							}
							else if (temp instanceof Worker) {
								System.out.println("\n Worker\'s hourly rate: "+((Worker) temp).getHourlyRate());
							}
							foundEmp=true;
							break;
						}
